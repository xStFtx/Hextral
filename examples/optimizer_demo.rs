...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, optimizer) in optimizers {
        ...
        let (loss_history, _) = nn.train(&inputs, &targets, 1.0, epochs, None, None, None, None, None).await?;
        ...
        for (i, input) in inputs.iter().enumerate() {
            ...
            println!("  [{:.0}, {:.0}] -> Expected: {:.0}, Predicted: {:.3}, Error: {:.3}", 
                    input[0], input[1], expected, predicted, (expected - predicted as f64).abs());
        }
        println!();
    }
