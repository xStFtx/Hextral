use hextral::{
    dataset::image::{AugmentationConfig, ImageLoader, LabelStrategy},
    ActivationFunction, Hextral, HextralResult, Optimizer,
};
...
async fn main() -> HextralResult<()> {
    ...
    println!(
        "   Images: {} samples",
        grayscale_dataset.metadata.sample_count
    );
    println!(
        "   Pixels per image: {} (28×28×1)",
        grayscale_dataset.metadata.feature_count
    );
    ...
    println!(
        ...
        train_set.metadata.sample_count, test_set.metadata.sample_count
    );

    // Convert targets to one-hot encoding
    let num_classes = train_set
        .target_names
        .as_ref()
        .map(|names| names.len())
        .unwrap_or(2);
    ...
    let one_hot_targets: Vec<DVector<f64>> = train_targets
        .iter()
        .map(|target| {
    ...
    for (i, (input, target)) in test_inputs.iter().zip(test_targets.iter()).enumerate() {
        ...
        if i < 3 {
            println!(
                "   Sample {}: predicted {}, actual {} (confidence: {:.2})",
                i + 1,
                predicted_class,
                actual_class,
    ...
    if sample_image_path.exists() {
        ...
        if let Some(class_names) = &dataset.target_names {
            if predicted_class < class_names.len() {
                println!(
                    "Predicted class for {}: {} (confidence: {:.2})",
...
fn create_sample_image(r: u8, g: u8, b: u8) -> RgbImage {
    ...
    ImageBuffer::from_fn(width, height, |x, y| {
        ...
        let pattern_g =
            ((g as f32 * (1.0 + 0.2 * (y as f32 / height as f32).sin())) as u8).min(255);
        let pattern_b =
            ((b as f32 * (1.0 + 0.1 * ((x + y) as f32 / (width + height) as f32).sin())) as u8)
                .min(255);
