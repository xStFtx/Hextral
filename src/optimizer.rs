...
impl Optimizer {
    ...
    fn adam_update(
        ...
        // Update parameters
        let weight_update =
            m_hat_weights.component_div(&(v_hat_weights.map(|x| x.sqrt() + epsilon)));
    ...
    fn rmsprop_update(
        ...
        // Update parameters
        let weight_update = weight_grad
            .component_div(&state.squared_weights[layer_idx].map(|x| x.sqrt() + epsilon));
        let bias_update =
            bias_grad.component_div(&state.squared_biases[layer_idx].map(|x| x.sqrt() + epsilon));
    ...
    fn adagrad_update(
        ...
        // Update parameters
        let weight_update = weight_grad
            .component_div(&state.accumulated_weights[layer_idx].map(|x| x.sqrt() + epsilon));
        let bias_update = bias_grad
            .component_div(&state.accumulated_biases[layer_idx].map(|x| x.sqrt() + epsilon));
    ...
    fn adadelta_update(
        ...
        let weight_update = weight_grad
            .component_mul(&rms_delta_weights)
            .component_div(&rms_grad_weights);
        let bias_update = bias_grad
            .component_mul(&rms_delta_biases)
            .component_div(&rms_grad_biases);
    ...
    fn nadam_update(
        ...
        // Nesterov momentum
        let m_hat_weights = (&state.velocity_weights[layer_idx] * beta1
            + weight_grad * (1.0 - beta1))
            / bias_correction1;
        let m_hat_biases = (&state.velocity_biases[layer_idx] * beta1 + bias_grad * (1.0 - beta1))
            / bias_correction1;
    ...
    fn adabelief_update(
        ...
        // Update parameters
        let weight_update =
            m_hat_weights.component_div(&(s_hat_weights.map(|x| x.sqrt() + epsilon)));
