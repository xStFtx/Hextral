...
impl MemoryPool {
    ...
    pub fn memory_usage(&self) -> MemoryUsage {
        let vector_memory: usize = self.vector_pool.iter()
            .map(|v| v.len() * std::mem::size_of::<f64>())
            .sum();
            
        let matrix_memory: usize = self.matrix_pool.iter()
            .map(|m| m.len() * std::mem::size_of::<f64>())
            .sum();
            
        let batch_memory: usize = self.batch_pool.iter()
...
impl BatchProcessor {
    ...
    pub fn process_batch_sync<F>(&mut self, 
                                inputs: &[DVector<f64>], 
                                targets: &[DVector<f64>], 
                                mut processor: F) -> HextralResult<Vec<f64>>
    where
        F: FnMut(&DVector<f64>, &DVector<f64>) -> HextralResult<f64>,
    {
        if inputs.len() != targets.len() {
            return Err(HextralError::InvalidInput {
                ...
                details: format!("Input batch size {} doesn't match target batch size {}", 
    ...
    pub async fn process_chunked<F, R>(&mut self, 
                                      inputs: &[DVector<f64>], 
                                      mut processor: F) -> HextralResult<Vec<R>>
    where
    ...
    pub fn compute_batch_stats(&self, data: &[DVector<f64>]) -> HextralResult<BatchStats> {
        ...
        for sample in data {
            if sample.len() != dim {
                return Err(HextralError::InvalidInput {
                    context: "Batch statistics".to_string(),
                    details: format!("Inconsistent dimensions: expected {}, got {}", dim, sample.len()),
                    recoverable: true,
                });
            }
    ...
    pub fn recommend_batch_size(&self, 
                               sample_size: usize, 
                               available_memory_mb: usize, 
                               model_parameters: usize) -> usize {
