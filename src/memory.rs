...
impl MemoryManager {
    ...
    pub fn matrix_multiply_inplace(
        ...
        if left.ncols() != right.nrows() {
            return Err(HextralError::InvalidInput {
                ...
                details: format!("Incompatible dimensions: {}x{} * {}x{}", 
    ...
    pub fn vector_add_inplace(&self, target: &mut DVector<f64>, source: &DVector<f64>) -> HextralResult<()> {
    ...
    pub fn memory_stats(&self) -> MemoryStats {
        let vector_pool_memory: usize = self.vector_pools.iter()
            .map(|(&size, pool)| size * pool.len() * std::mem::size_of::<f64>())
            .sum();
            
        let matrix_pool_memory: usize = self.matrix_pools.iter()
    ...
    pub fn force_cleanup(&mut self) {
        ...
        if self.allocation_history.len() > 1000 {
            self.allocation_history.drain(..self.allocation_history.len() / 2);
        }
    ...
    pub fn analyze_patterns(&self) -> AllocationAnalysis {
        ...
        AllocationAnalysis {
            most_common_vector_size: vector_sizes.iter()
                .max_by_key(|(_, count)| *count)
                .map(|(size, _)| *size),
            most_common_matrix_size: matrix_sizes.iter()
                .max_by_key(|(_, count)| *count)
                .map(|(size, _)| *size),
...
impl GradientStorage {
    ...
    pub fn accumulate(&mut self, other: &GradientStorage) -> HextralResult<()> {
        ...
        for (self_grad, other_grad) in self.weight_gradients.iter_mut().zip(&other.weight_gradients) {
...
mod tests {
    ...
    fn test_in_place_operations() {
        ...
        manager.matrix_multiply_inplace(&mut result, &left, &right).unwrap();
