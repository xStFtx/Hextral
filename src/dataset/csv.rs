...
impl CsvLoader {
    ...
    pub async fn from_string(&self, data: &str) -> Result<Dataset, DatasetError> {
        ...
        let headers = if self.has_headers {
            match csv_reader.headers() {
                Ok(h) => Some(h.clone()),
                Err(e) => {
                    return Err(DatasetError::CsvError(format!(
        ...
        let metadata = DatasetMetadata {
            ...
            target_count: if targets.is_some() {
                Some(column_info.target_columns.len())
            } else {
                None
            },
            source: None,
    ...
    fn analyze_columns<R>(
        &self,
        headers: &Option<StringRecord>,
        csv_reader: &mut csv::Reader<R>,
    ) -> Result<ColumnInfo, DatasetError>
    where
        R: std::io::Read,
    {
        ...
        let target_indices = match &self.target_columns {
            ...
            TargetColumns::Last(n) => {
                if *n > total_columns {
                    return Err(DatasetError::Configuration(format!(
                        "Cannot use last {} columns as targets, only {} columns available",
                        n, total_columns
                    )));
                }
    ...
    async fn process_records<R>(
        &self,
        mut csv_reader: csv::Reader<R>,
        column_info: &ColumnInfo,
    ) -> Result<(Vec<DVector<f64>>, Option<Vec<DVector<f64>>>, DatasetStats), DatasetError>
    where
        R: std::io::Read,
    {
        let mut features = Vec::new();
        let mut targets = if column_info.target_columns.is_empty() {
            None
        } else {
            Some(Vec::new())
        };
    ...
    fn parse_value(&self, value: &str) -> Result<f64, DatasetError> {
        if self.missing_values.iter().any(|mv| mv == value) {
            ...
            match value.to_lowercase().as_str() {
                "true" | "yes" | "1" | "y" => Ok(1.0),
                "false" | "no" | "0" | "n" => Ok(0.0),
                _ => Err(DatasetError::Parse(format!(
